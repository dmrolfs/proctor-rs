...
use crate::elements::PolicyOutcome;
use crate::flink::governance::context::FlinkGovernanceContext;
use crate::flink::plan::FlinkScalePlan;
use crate::graph::stage;
use crate::graph::stage::ThroughStage;

type Item = FlinkScalePlan;
type Context = FlinkGovernanceContext;

pub fn make_governance_transform(name: impl AsRef<str>) -> impl ThroughStage<PolicyOutcome<Item, Context>, Item> {
    let stage = stage::FilterMap::new(name, move |outcome: PolicyOutcome<Item, Context>| {
        ...
        match (
            ...
            (Ok(Some(adjusted)), _current, _target) => {
                ...
                Some(FlinkScalePlan { target_nr_task_managers: adjusted, ..outcome.item })
            },
        }
    });
